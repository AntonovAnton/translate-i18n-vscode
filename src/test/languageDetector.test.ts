...
// Import the LanguageDetector and related types
...
suite('LanguageDetector Test Suite', () => {
	...
	suite('Language Detection from Project', () => {
		...
		test('handles case-insensitive language detection', () => {
			...
			assert.deepStrictEqual(
				detectedLanguages.sort((a: string, b: string) => a.localeCompare(b, undefined, { sensitivity: 'base' })),
				['de', 'Fr', 'RU'].sort((a: string, b: string) => a.localeCompare(b, undefined, { sensitivity: 'base' }))
			);
		});
	});
});
